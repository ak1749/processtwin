...
import { Check, Copy, LayoutTemplate, Network, Redo2, Sparkles, Undo2, Workflow, X } from 'lucide-react';
...
export default function WorkspacePage() {
  ...
  return (
    <main className="grid h-[100dvh] min-w-0 grid-rows-[72px_minmax(0,1fr)_220px] overflow-hidden bg-slate-100/70 text-slate-950">
      <header className="flex items-center justify-between border-b border-slate-200 bg-white px-5 shadow-[0_1px_2px_rgb(15_23_42/0.03)]">
        <div className="flex min-w-0 items-center gap-3"><span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-white shadow-[0_2px_5px_rgb(15_23_42/0.16)]"><Workflow size={18} /></span><div className="min-w-0"><p className="text-xs font-medium text-slate-500">ProcessTwin workspace</p><h1 className="truncate text-base font-semibold tracking-[-0.02em] text-slate-950">{process.name}</h1></div></div>
        <div className="flex items-center gap-2" aria-label="Workspace controls">
          <button type="button" onClick={() => undoProcess({ actor: 'human' }, {})} disabled={past.length === 0} className="pt-subtle-control" aria-label="Undo last edit"><Undo2 size={15} /> Undo</button>
          <button type="button" onClick={() => redoProcess({ actor: 'human' }, {})} disabled={future.length === 0} className="pt-subtle-control" aria-label="Redo last edit"><Redo2 size={15} /> Redo</button>
          <button type="button" onClick={applyAutoLayout} disabled={visibleProcess.nodes.length === 0} className="pt-subtle-control" aria-label="Lay out process from left to right"><LayoutTemplate size={15} /> Layout</button>
          <button type="button" onClick={() => setActiveScenarioId(null)} disabled={!activeScenario} className="pt-subtle-control" aria-label="Switch to main process">{activeScenario ? 'View main' : 'Main process'}</button>
          <button type="button" onClick={() => setShowArchitecture(true)} className="pt-subtle-control" aria-label="Show architecture card"><Network size={15} /> Architecture</button>
          <button type="button" onClick={() => setActiveTab('Simulation')} className="pt-control" aria-label="Open simulation drawer">Simulate</button>
        </div>
      </header>
      ...
      <section className="min-h-0 min-w-0 border-t border-slate-200 bg-white shadow-[0_-1px_2px_rgb(15_23_42/0.02)]" aria-label="Workspace drawer">
        <div className="flex h-12 items-center gap-1 border-b border-slate-100 px-4" role="tablist" aria-label="Workspace drawer tabs">
          {drawerTabs.map((tab) => <button key={tab} type="button" role="tab" aria-selected={activeTab === tab} onClick={() => setActiveTab(tab)} className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${activeTab === tab ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}>{tab}</button>)}
        </div>
