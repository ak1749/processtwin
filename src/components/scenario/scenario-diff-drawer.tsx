...
import type { BusinessProcess, Operator, ProcessConnection } from '../../types/process';
...
type ChangeTone = 'added' | 'modified' | 'removed';
...
function stepName(process: BusinessProcess, id: string): string {
  return process.nodes.find((step) => step.id === id)?.name ?? id;
}
...
function formatCondition(edge: ProcessConnection, process: BusinessProcess): string {
  if (!edge.condition) return 'no condition';
  const variable = process.variables.find((entry) => entry.key === edge.condition?.variable);
  const operator: Record<Operator, string> = {
    eq: '=',
    neq: '≠',
    gt: '>',
    gte: '≥',
  ...
  return `${variable?.label ?? edge.condition.variable} ${operator[edge.condition.operator]} ${formatValue(edge.condition.value)}`;
}

function describeEdge(edge: ProcessConnection, process: BusinessProcess): string {
  const route = `${stepName(process, edge.source)} → ${stepName(process, edge.target)}`;
  if (edge.condition) return `${route} when ${formatCondition(edge, process)}`;
  if (edge.probability !== undefined) return `${route} with ${(edge.probability * 100).toFixed(0)}% probability`;
  return route;
}

function edgeModificationText(
  before: ProcessConnection,
  after: ProcessConnection,
  fields: Array<'source' | 'target' | 'label' | 'condition' | 'probability'>,
  process: BusinessProcess,
): string {
  const details = fields.map((field) => {
    if (field === 'condition') return `condition: ${formatCondition(before, process)} → ${formatCondition(after, process)}`;
    if (field === 'probability') return `probability: ${before.probability ?? 'none'} → ${after.probability ?? 'none'}`;
    if (field === 'label') return `label: ${before.label ?? 'none'} → ${after.label ?? 'none'}`;
    if (field === 'source') return `from: ${stepName(process, before.source)} → ${stepName(process, after.source)}`;
    return `to: ${stepName(process, before.target)} → ${stepName(process, after.target)}`;
  });
  return `Modified: ${describeEdge(after, process)} — ${details.join('; ')}`;
}
...
export function ScenarioDiffDrawer() {
  ...
  const changes: ChangeEntry[] = [
    ...(diff?.edgesModified ?? []).map((change) => ({
      tone: 'modified' as const,
      text: edgeModificationText(change.before, change.after, change.changedFields, main),
    })),
    ...(diff?.edgesAdded ?? []).map((edge) => ({ tone: 'added' as const, text: `Added: ${describeEdge(edge, scenario.process)}` })),
    ...(diff?.edgesRemoved ?? []).map((edge) => ({ tone: 'removed' as const, text: `Removed: ${describeEdge(edge, main)}` })),
    ...(diff?.modified ?? []).map((change) => ({ tone: 'modified' as const, text: `Modified: ${stepName(main, change.stepId)} — ${change.field}` })),
    ...(diff?.added ?? []).map((step) => ({ tone: 'added' as const, text: `Added: ${step.name}` })),
    ...(diff?.removed ?? []).map((step) => ({ tone: 'removed' as const, text: `Removed: ${step.name}` })),
  ];
